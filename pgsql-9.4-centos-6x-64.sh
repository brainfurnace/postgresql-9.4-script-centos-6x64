...
function install_postgresql(){
	...
	#3. Install PostgreSQL along with perl-DBD-Pg for webmin
	...
	#6. Add Postgre variables to environment
...
CMD_EOF
	...
	#8. Enable Postgre to start on boot
	chkconfig --level 234 postgresql-9.4 on

	#9. Create Symlinks for Backward Compatibility from PostgreSQL 9 to PostgreSQL 8
	...
	#10. create self-signed SSL certificates
...
#13. Set webmin configuration to work with 9.x file structure
...
max_text=1000
perpage=25
stop_cmd=if [ -r /etc/rc.d/init.d/rhdb ]; then /etc/rc.d/init.d/rhdb stop; else /etc/rc.d/init.d/postgresql-9.4 stop; fi
psql=/usr/bin/psql
pid_file=/var/run/postmaster-9.4.pid
hba_conf=/var/lib/pgsql/9.4/data/pg_hba.conf
setup_cmd=if [ -r /etc/rc.d/init.d/rhdb ]; then /etc/rc.d/init.d/rhdb start; else /etc/rc.d/init.d/postgresql-9.4 initdb ; /etc/rc.d/init.d/postgresql-9.4 start; fi
user=postgres
nodbi=0
max_dbs=50
start_cmd=if [ -r /etc/rc.d/init.d/rhdb ]; then /etc/rc.d/init.d/rhdb start; else /etc/rc.d/init.d/postgresql-9.4 start; fi
...
add_mode=1
blob_mode=0
pass=NR7toiBIxa43usbueAywqsdDMvpg19wJ
plib=
encoding=
port=
host=
...
#14. Display pgadmin, postgres, root, and ssl password and save to auth.txt file
